package com.oldhammer.fantasylistbuilder.auth.service;
...
import com.oldhammer.fantasylistbuilder.helpers.CookieUtils;
import com.oldhammer.fantasylistbuilder.models.user.CustomOAuth2User;
import com.oldhammer.fantasylistbuilder.services.userServices.UserService;
...
import static com.oldhammer.fantasylistbuilder.constants.CookiesName.REDIRECT_URI_PARAM_COOKIE_NAME;
        ...
        switch (provider) {
            case "github":
                userService.createOauth2User(new UserDTO("NoMail goten from #" + userName, userName, attributes.get("login").toString(),"", null,
                        null, null, "ROLE_USER", 0.), Provider.GITHUB);
                break;
            case "google":
                userService.createOauth2User(new UserDTO(attributes.get("email").toString(), userName, attributes.get("name").toString(), "", attributes.get("given_name").toString(),
                        attributes.get("family_name").toString(), null, "ROLE_USER", 0.), Provider.GOOGLE);
                break;
            default:
